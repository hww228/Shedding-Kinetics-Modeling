...
vir_exponential <- function(tm,ap,cp){
  return(cp/log(10)+log10(exp(-ap*tm)));
}

vir_gamma <- function(tm,ap,bp,cp){
  return(cp/log(10)+log10(tm^bp)+log10(exp(-ap*tm)));
}


quant.resp <- function(k.subj,t,model){
  if (model=="exp"){
    tmp <- vir_exponential(t,a0,c0);
  }
  if (model=="gamma"){
    tmp <- vir_gamma(t,a0,b0,c0);
  }
  tmp <- as.numeric(na.omit(tmp));
  return(quantile(tmp,probs=c(0.025,0.5,0.975)));
}

graph.resp <- function(k.subj,color,model){
  x <- seq(from=0,to=30,by=0.2);
  y <- array(NA,dim=c(length(x),3));
  for(k in 1:length(x)){
    y[k,] <- quant.resp(k.subj,x[k],model);
  }
...
graph.obs <- function(dat,k.subj,colr="black"){
  points(dat$t_obs[dat$p_obs==k.subj],dat$c_obs[dat$p_obs==k.subj],col="black",cex=2,pch=16)
  points(dat$t_cen[dat$p_cen==k.subj],rep(dat$censlim,length(dat$t_cen[dat$p_cen==k.subj])),col="black",cex=2,pch=1)
}

quant.resp.1 <- function(t,model){
  if (model=="exp"){
    tmp <- vir_exponential(t,a0,c0);
  }
  if (model=="gamma"){
...
graph.resp.1 <- function(color,model){
  x <- seq(from=0,to=30,by=0.2);
  y <- array(NA,dim=c(length(x),3));
  for(k in 1:length(x)){
    y[k,] <- quant.resp.1(x[k],model);
  }
  lines(x,y[,1],col=color,lty=3,lwd=2);
  lines(x,y[,2],col=color,lty=1,lwd=2);
  lines(x,y[,3],col=color,lty=3,lwd=2);
